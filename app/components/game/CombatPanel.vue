    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 relative z-10">
      <div class="p-3 bg-gray-800/60 rounded border border-green-600/40 space-y-2 relative">
        ...
        <div class="w-full bg-blue-900/70 rounded-full h-3 relative overflow-hidden border border-blue-700/50">
          ...
          <div class="bg-gradient-to-r from-blue-500 to-sky-400 h-full rounded-full transition-all duration-300 ease-in-out shadow-inner" :style="{ width: `${playerEnergyPercent}%` }" />
        </div>
      </div>
      ...
      <div class="p-3 bg-gray-800/60 rounded border border-red-600/40 space-y-2 relative">
        ...
        <p class="text-base md:text-lg text-red-300 font-semibold truncate" :title="monster.name">
          {{ monster.name }}
        </p>
        ...
        <div class="w-full bg-red-900/70 rounded-full h-3 mt-1 relative overflow-hidden border border-red-700/50">
          ...
          <div class="bg-gradient-to-r from-red-500 to-orange-400 h-full rounded-full transition-all duration-300 ease-in-out shadow-inner" :style="{ width: `${monsterHpPercent}%` }" />
        </div>
      </div>
    </div>

    <div class="flex justify-center items-center gap-3 md:gap-4 pt-3 relative z-10 border-t border-gray-700/50">
      <button
        class="bg-gradient-to-b from-red-600 to-red-800 hover:from-red-500 hover:to-red-700 text-white px-4 py-2 rounded-lg font-semibold border-2 border-red-500/80 shadow-lg disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center transform hover:scale-105 transition-transform min-w-[110px]"
        :disabled="isLoading"
        @click="performAttack"
      >
        <UiLoadingSpinner v-if="isLoading" class="h-5 w-5 mr-2"/>
        <Icon v-else name="lucide:sword" class="h-5 w-5 mr-1.5"/>
        <span>Tấn Công</span>
      </button>

      <button class="bg-gradient-to-b from-gray-600 to-gray-800 text-gray-400 px-4 py-2 rounded-lg font-semibold border-2 border-gray-500/80 shadow-md opacity-60 cursor-not-allowed flex items-center justify-center min-w-[110px]">
        <Icon name="lucide:sparkles" class="h-5 w-5 mr-1.5"/> Kỹ Năng
      </button>

      <button class="bg-gradient-to-b from-blue-600 to-blue-800 hover:from-blue-500 hover:to-blue-700 text-white px-4 py-2 rounded-lg font-semibold border-2 border-blue-500/80 shadow-lg flex items-center justify-center transform hover:scale-105 transition-transform min-w-[110px]">
         <Icon name="lucide:shield-off" class="h-5 w-5 mr-1.5"/> Bỏ Chạy
      </button>
    </div>
  </div>
